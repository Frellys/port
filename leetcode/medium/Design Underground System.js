...
UndergroundSystem.prototype.checkIn = function (id, stationName, t) {
    this.customers.set(id, { name: stationName, time: t });
};
...
UndergroundSystem.prototype.checkOut = function (id, stationName, t) {
    let prev = this.customers.get(id);
    let route = prev.name + '_' + stationName;
    let time = t - prev.time;
    if ((route in this.routes) == false) {
        this.routes[route] = [];
    }
    this.routes[route].push(time);
...
UndergroundSystem.prototype.getAverageTime = function (startStation, endStation) {
    ...
    return this.routes[route].reduce((acc, cur) => { return acc + cur }, 0) / this.routes[route].length;
};
