...
// where < col1 > represents the column c1, <row1> represents the row r1, <col2> represents the column c2, and <row2> represents the row r2, such that r1 <= r2 and c1 <= c2.
...
var cellsInRange = function (s) {
    let [a, b] = s.split(':');
    let ca = a[0].charCodeAt(0);
    let ra = +a.slice(1);
    let cb = b[0].charCodeAt(0);
    let rb = +b.slice(1);
    let ret = [];
    for (let c = ca; c <= cb; c++) {
        let col = String.fromCharCode(c);
        for (let r = ra; r <= rb; r++) {
            ret.push(col + r);
        }
    }
