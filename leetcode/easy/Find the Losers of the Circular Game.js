...
var circularGameLosers = function (n, k) {
    let arr = new Array(n).fill(false).map((el, edx) => {
        return { num: edx + 1, seen: (+!edx) };
    });
    let step = 1;
    let pos = 0;
    while (arr.every(el => el.seen < 2)) {
        arr[pos = (pos + step++ * k) % arr.length].seen++;
    }
    return arr.filter(el => !el.seen).map(el => el.num);
};