...
var numberOfPairs = function (nums) {
    let seen = Object.fromEntries(Array.from(Array.from(new Set(nums)).map(n => [n, 0])));
    nums.forEach(n => seen[n]++);
    return [
        Object.entries(seen).reduce((acc, [key, val]) => {
            seen[key] = (val % 2);
            return acc + Math.floor(val / 2);
        }, 0),
        Object.values(seen).filter(v => v).length
    ];
};